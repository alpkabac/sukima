...
class PromptService {
    ...
    static getPrompt(msg, from, channel, usesIntroduction = true, usesHistory = true, isContinuation = false) {
        ...
        let filter = false
        return this.getIntroduction(usesIntroduction)
                .concat(
                    this.getChannelMemory(channel),
                    !usesHistory ?
                        [{from: conf.botName, msg: translationService.botTranslations.noContextSentence}, {from, msg}]
                        : historyService

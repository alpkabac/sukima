        ...
        let text = Object.keys(players)
            ...
            .map(playerName =>
                ...
                + players[playerName].inventory
                    ...
                    .map((item, index) =>
                        `${players[playerName].inventory.indexOf(item).toString().padEnd(3, ' ')}: [${item.rarity} ${item.type}] "${item.name}"${item.image ? ' :white_check_mark:' : ''}${item.forSale ? ` (:moneybag: ${item.playerPrice} ${rpgService.getCurrency()})` : ''}`
                    )
                    .join('\n')
            )
            .join('\n')

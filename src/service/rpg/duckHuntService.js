...
class DuckHuntService {
    ...
    static async sell(channel, username, itemSlot) {
        ...
        if (!merchant) {
            return {
                ...
                deleteUserMsg: username !== process.env.BOTNAME,
                deleteNewMessage: username !== process.env.BOTNAME,
                pushIntoHistory: username !== process.env.BOTNAME ? null : [`[ ${username} tried to sell an item, but there is no merchant around. ]`, null, channel],
            }
        }
    ...
    static async showAllShops(channel, username) {
        ...
        let text = Object.keys(players)
            ...
            .map(playerName =>
                `**${playerName !== travellingMerchantService.getMerchantName() ? 'Player ' : ''}${playerName}'s shop:**\n`
                + players[playerName].inventory
                    .filter(item => item.forSale)
                    .map(item =>
                        `Slot [${players[playerName].inventory.indexOf(item)}]${item.image ? ' :frame_photo:' : ''}\nItem: ${item.name} (${item.rarity} ${item.type})\nPrice: ${item.playerPrice} ${rpgService.getCurrency()}`
                    )
                    .join('\n')
            )
            .join('\n\n')
